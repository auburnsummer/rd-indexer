...
def test_fill_in_params_number():
    assert (
        fill_in_params("SELECT * FROM orchard WHERE param = ?", [2])
        == "SELECT * FROM orchard WHERE param = 2"
    )
...
async def test_datasette_request(empty_db, datasette_responses):
    Level.bind(empty_db)
    result = await datasette_request(
        Level.select().where(Level.artist == "auburnsummer")
    )
    for r in result:
...
async def test_datasette_request_empty(empty_db, datasette_responses):
    Level.bind(empty_db)
    result = await datasette_request(
        Level.select().where(Level.artist == "fjaiwpejpawjopfawefawe")
    )
    assert result == []
