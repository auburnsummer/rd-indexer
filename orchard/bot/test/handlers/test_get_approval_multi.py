...
def test_client():
    app = Starlette(
        routes=[Route("/", get_approval_multi.__wrapped__, methods=["POST"])]
    )
    tc = TestClient(app)
    return tc
...
async def test_get_approval_multi(empty_db_with_status_bind, test_client):
    ...
    assert resp.json() == [
        {"approval": 0, "approval_reasons": None, "id": "a", "indexed": None},
        {"approval": 0, "approval_reasons": None, "id": "b", "indexed": None},
        {
            "approval": 10,
...
@pytest.mark.asyncio
async def test_get_approval_doesnt_already_exist(
    empty_db_with_status_bind, test_client
):
    resp = test_client.post("/", json=["a", "b", "c"])
    assert resp.status_code == 200
    assert resp.json() == [
        {"approval": 0, "approval_reasons": None, "id": "a", "indexed": None},
        {"approval": 0, "approval_reasons": None, "id": "b", "indexed": None},
        {"approval": 0, "approval_reasons": None, "id": "c", "indexed": None},
    ]
