...
def main(db: Database):
    ...
    for source_id, scraper_class in sources:
        ...
        logger.info(f"Adding {len(iids_to_add)} iids, removing {len(iids_to_delete)} iids")
        ...
        for index, iid in enumerate(iids_to_add):
            try:
                ...
                vit = analyze(file)
                logger.info(f"it's {vit['song']} by {','.join(vit['authors'])}, id: {vit['id']}")
